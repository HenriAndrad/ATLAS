...
from app.services.auth_service import create_token, hash_password, is_email_domain_allowed, verify_password
...
async def register(
    ...
    if not is_email_domain_allowed(payload.email):
        raise HTTPException(
            status_code=422,
            detail=f"Só e-mails terminados em {settings.allowed_email_domain} podem se cadastrar.",
        )
